...
export default function EsppDepositRow2(props) {
    ...
    const dispatchAmountGainAndTotalLiab =  useEffect(() => {
        debugger;
        dispatch(updateEsppRow({id: props.id, units: units, amountGain: amountGain, totalLiab: totalLiab }))
        setTotalChargeableAmount(calcTotalChargeableAmount(props.row.date,esppRowData, totalLiab))
    ...
    const handleUnitsChange = (event) => {
        setUnits(parseInt(event.target.value)) //no ()=> as not working with previou value, just replacing with new value
