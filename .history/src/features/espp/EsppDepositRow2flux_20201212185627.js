...
export default function EsppDepositRow2(props) {
    ...
    const calcTotalChargeableAmount = (dateNotToInclude, esppRowData, currentTotalLiab) =>{
        esppRowData.filter((currentRow)=>  currentRow.date != dateNotToInclude && moment(currentRow.date).format("YY").equals(moment(dateNotToInclude).format("YY"))
        )
        // const dresult = esppRowData.map((result.totalLiab)=> )

        const tempamountGain = dresult.reduce( (acc, currRow)=>{
    ...
    const updateTotalLiab =  useEffect(() => {
        ...
        setTotalChargeableAmount(calcTotalChargeableAmount(props.date,esppRowData, totalLiab))
