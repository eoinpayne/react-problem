...
export default function EsppDepositRow2(props) {
    ...
    //     const tempamountGain = groupedRows.reduce( (acc, currRow)=>{
    //         // acc += currRow.amountGain
    //     },0)
    ...
    const updateTotalLiab =  useEffect(() => {
        ...
        // setTotalChargeableAmount(calcTotalChargeableAmount())
    ...
    return (
        <tr>
            <td>{props.index}</td>
            <td>{moment(props.row.date).format("YYYY-MM-DD")}</td>qasdasd
