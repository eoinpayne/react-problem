...
export default function EsppDepositRow2(props) {
    ...
    const calcTotalChargeableAmount = (dateNotToInclude, esppRowData, currentTotalLiab) =>{
        const filteredRows = esppRowData.filter((currentRow)=>  currentRow.date != dateNotToInclude && (moment(currentRow.date).format("YY")).isSame(moment(dateNotToInclude).format("YY"))
        )
